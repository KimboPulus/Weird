...
public enum ToolMode {
    RAIN("Rain", "Water a 4 x 4 patch and cool it a little. Repeated use can flood the world.", 0) {
        @Override
        public boolean apply(Simulation simulation, Position position) {
            return simulation.rain(position);
        }
    },
    DROUGHT("Drought", "Dry a 4 x 4 patch and warm it a little. Repeated use can kill the soil.", 0) {
        @Override
        public boolean apply(Simulation simulation, Position position) {
    ...
    LIGHTNING("Lightning", "Strike one exact creature with blue lightning for 50 tokens.", 50) {
        @Override
        public boolean apply(Simulation simulation, Position position) {
